...
GLUON_RESNET_TORCH_HUB = '/home/juliuxue/.cache/torch/hub/rwightman_pytorch-pretrained-gluonresnet_master'
...
def resnet34_v1b(pretrained=False, **kwargs):
    ...
    if pretrained:
        ...
        filtered_orig_dict = _safe_state_dict_filtering(
            torch.hub.load(GLUON_RESNET_TORCH_HUB, 'gluon_resnet34_v1b', pretrained=True,source='local').state_dict(),
            model_dict.keys()
        )
        model_dict.update(filtered_orig_dict)
        model.load_state_dict(model_dict)
        print("loda...pretrained model")
    return model
