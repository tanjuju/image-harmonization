...
def save_img(path, img):
    pred_rgb = Image.fromarray(img)
    pred_rgb.save(path, quality=100)
...
def evaluateModel(epoch_number, model, opt, test_dataset, epoch, max_psnr, iters=None):
    ...
    for i, data in tqdm(enumerate(test_dataset)):
        ...
        for i_img in range(real.size(0)):
            ...
            if epoch >= 110:
                ...
                save_img(
                    os.path.join('evaluate', str(epoch_number), 'results', basename, imagename.split('.')[0] + '.jpg'),
                    pred_rgb)
    ...
    all_mse, all_psnr, all_fmse, all_ssim = calculateMean(eval_results['mse']), calculateMean(
        eval_results['psnr']), calculateMean(eval_results['fmse']), calculateMean(eval_results['ssim'])
...
def resolveResults(results):
    interval_metrics = {}
    mask, mse, psnr, fmse, ssim = np.array(results['mask']), np.array(results['mse']), np.array(
        results['psnr']), np.array(results['fmse']), np.array(results['ssim'])
...
if __name__ == '__main__':
    ...
    epoch_mse, epoch_psnr, epoch_interval_metrics = evaluateModel(110, model, opt, test_dataloader, 110,
                                                                      110)
