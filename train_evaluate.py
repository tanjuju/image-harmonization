...
def evaluateModel(epoch_number, model, opt, test_dataset, epoch, max_psnr, iters=None):
    ...
    for i, data in tqdm(enumerate(test_dataset)):
        ...
        for i_img in range(real.size(0)):
            ...
            fmse_score_op = mean_squared_error(util.tensor2im(pred), util.tensor2im(gt)) * 256*256/ fore_nums##########在计算前景1024应该是1024*1024而不是256*256
            ...
            if epoch >= 47:
                ...
                save_img(os.path.join('evaluate', str(epoch_number), 'results',basename, imagename.split('.')[0] + '_pre.png'), pred_rgb)
                ############保存合成图像 真实图像
                real_rgb=util.tensor2im(gt)
                save_img(
                    os.path.join('evaluate', str(epoch_number), 'results', basename, imagename.split('.')[0] + '_real.png'),
                    real_rgb)
                comp1=comp[i_img:i_img+1]
                comp_rgb = util.tensor2im(comp1)
                save_img(
                    os.path.join('evaluate', str(epoch_number), 'results', basename, imagename.split('.')[0] + '_comp.png'),
                    comp_rgb)
                mask1=mask[i_img:i_img+1]
                mask_rgb =util.tensor2im(mask1)
                save_img(
                    os.path.join('evaluate', str(epoch_number), 'results', basename, imagename.split('.')[0] + '_mask.png'),
                    mask_rgb)
                ############
            ...
            eval_results_fstr.writelines('%s,%.3f,%.3f,%.3f,%.3f\n' % (data['img_path'][i_img], eval_results['mask'][-1],mse_score_op, psnr_score_op,fmse_score_op))
        if i + 1 % 100 == 0:
    ...
    print(len(eval_results['mse']))
    print('MSE:%.4f, PSNR:%.4f, fMSE:%.4f, SSIM:%.5f' % (all_mse, all_psnr, all_fmse, all_ssim))
    model.netG.train()
...
if __name__ == '__main__':
    ...
    opt.isTrain=False
    print("is training... ",opt.isTrain)
    model = create_model(opt)      # create a model given opt.model and other options
    setup = model.setup(opt)
    ############加载预训练模块######################
    # model.netG.module.backbone.load_pretrained_weights()
    # print(model.netG)
    ##########################################
    x1 = torch.randn(1, 4, 256, 256).to(torch.device('cuda:0'))
    x2 = torch.randn(1, 1, 256, 256).to(torch.device('cuda:0'))
    # # print(model.netG.module.device)
    #
    flost, params = profile(model.netG.module, inputs=(x1, x2))
    print('flost:', flost)
    print('params:', params)
    # epoch_mse, epoch_psnr, epoch_interval_metrics =evaluateModel(60, model, opt, test_dataloader, 60, 0)
    exit()
